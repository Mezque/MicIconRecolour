    ...
    internal class Main : MelonMod
    {
        ...
        public override void OnPreferencesSaved()
        {
            if (UIManagerInitialized && Modules.Prefs.MicRgb.Value)
            {
                ...
                icon.color = new(float.Parse(Modules.Prefs.MicColourR.Value) / 255f, float.Parse(Modules.Prefs.MicColourG.Value) / 255f, float.Parse(Modules.Prefs.MicColourB.Value) / 255f, float.Parse(Modules.Prefs.MicColourA.Value) / 255f);
            }
        }
        ...
        public override void OnUpdate()
        {
            ...
            icon.color = new((float)Math.Sin(2 * Time.time * 0.5f + 0.5f), (float)Math.Sin((2 * Time.time) + Math.PI * 2.0 / 3.0) * 0.5f + 0.5f, (float)Math.Sin((2 * Time.time) + Math.PI * 4.0 / 3.0) * 0.5f + 0.5f);
        }
    }
}